...
// ==========================================
// 8. GERENCIAMENTO DE EQUIPAMENTOS
// ==========================================
async function carregarEquipamentos(container) {
    container.innerHTML = `<div class="p-4 text-center text-slate-500"><i class="fa-solid fa-spinner fa-spin"></i> Carregando equipamentos...</div>`;
    ...
    container.innerHTML = `
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 w-full">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h3 class="font-bold text-slate-900 text-base">Controle de Equipamentos</h3>
                    <p class="text-xs text-slate-500">Gerencie o patrimônio de hardware da empresa.</p>
                </div>
                <button onclick="abrirModalNovoEquipamento()" class="btn btn-primario">
                    <i class="fa-solid fa-plus"></i> Novo Equipamento
                </button>
            </div>

            <div id="form-novo-equipamento-container" class="hidden mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <h4 class="font-bold text-slate-800 text-sm mb-3">Cadastrar Novo Equipamento</h4>
                <form id="form-equipamento" onsubmit="salvarEquipamento(event)" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <div>
                        <label class="rotulo">Patrimônio</label>
                        <input type="text" id="eq-patrimonio" class="campo" required placeholder="Ex: PAT-00123">
                    </div>
                    <div>
                        <label class="rotulo">Tipo</label>
                        <input type="text" id="eq-tipo" class="campo" required placeholder="Ex: Notebook / Impressora">
                    </div>
                    <div>
                        <label class="rotulo">Marca / Modelo</label>
                        <input type="text" id="eq-marcamodelo" class="campo" required placeholder="Ex: Dell Latitude 3420">
                    </div>
                    <div>
                        <label class="rotulo">Setor</label>
                        <input type="text" id="eq-setor" class="campo" required placeholder="Ex: Financeiro">
                    </div>
                    <div>
                        <label class="rotulo">Responsável</label>
                        <input type="text" id="eq-responsavel" class="campo" required placeholder="Nome do colaborador">
                    </div>
                    <div class="flex items-end">
                        <button type="submit" class="btn btn-primario w-full">Salvar Equipamento</button>
                    </div>
                </form>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs text-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ${(equipamentos || []).length > 0 ? equipamentos.map(e => `
                            <tr>
                                <td class="p-3 font-bold">#${escapeHTML(e.patrimonio)}</td>
                                <td class="p-3">${escapeHTML(e.tipo)}</td>
                                <td class="p-3">${escapeHTML(e.marca_modelo)}</td>
                                <td class="p-3">${escapeHTML(e.setor)}</td>
                                <td class="p-3">${escapeHTML(e.responsavel)}</td>
                                <td class="p-3"><span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-600">${escapeHTML(e.status)}</span></td>
                                <td class="p-3 text-right">
                                    <button onclick="deletarEquipamento(${e.id})" class="text-rose-500 hover:text-rose-700 px-2 py-1"><i class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>
                        `).join('') : '<tr><td colspan="7" class="p-4 text-center text-slate-400">Nenhum equipamento cadastrado.</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
function abrirModalNovoEquipamento() {
    const el = document.getElementById('form-novo-equipamento-container');
...
async function salvarEquipamento(event) {
    ...
    const marca_modelo = document.getElementById('eq-marcamodelo').value;
    ...
    const { error } = await supabaseClient.from('equipamentos').insert([{
        patrimonio, tipo, marca_modelo, setor, responsavel, status: 'Ativo'
    }]);
...
// ==========================================
// 9. GERENCIAMENTO DE CATEGORIAS DE PROBLEMA
// ==========================================
async function carregarCategorias(container) {
    container.innerHTML = `<div class="p-4 text-center text-slate-500"><i class="fa-solid fa-spinner fa-spin"></i> Carregando categorias...</div>`;

    const { data: categorias } = await supabaseClient.from('categorias_problema').select('*').order('id', { ascending: true });

    container.innerHTML = `
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 w-full">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="font-bold text-slate-900 text-base">Categorias de Chamados</h3>
                    <p class="text-xs text-slate-500">Defina os tipos de problemas e suas subcategorias.</p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ${(categorias || []).map(cat => `
                    <div class="p-4 border border-slate-200 rounded-xl bg-slate-50/50">
                        <h4 class="font-bold text-slate-900 text-sm mb-2"><i class="fa-solid fa-tag text-emerald-600 mr-2"></i>${escapeHTML(cat.nome)}</h4>
                        <div class="flex flex-wrap gap-1 mt-2">
                            ${(cat.subcategorias || []).map(sub => `<span class="bg-white border border-slate-200 text-slate-700 text-[11px] px-2.5 py-1 rounded-lg font-medium">${escapeHTML(sub)}</span>`).join('')}
                        </div>
                    </div>
                `).join('')}
            </div>
...
// ==========================================
// 10. GERENCIAMENTO DE SETORES E LIBERAÇÕES
// ==========================================
async function carregarSetores(container) {
    container.innerHTML = `<div class="p-4 text-center text-slate-500"><i class="fa-solid fa-spinner fa-spin"></i> Carregando setores...</div>`;

    const { data: setores } = await supabaseClient.from('setores_liberacoes').select('*').order('id', { ascending: true });

    container.innerHTML = `
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 w-full">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="font-bold text-slate-900 text-base">Setores e Níveis de Liberação</h3>
                    <p class="text-xs text-slate-500">Departamentos e respectivas permissões de alçada.</p>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs text-slate-700">
                    <thead class="bg-slate-50 uppercase text-[10px] text-slate-500">
                        <tr>
                            <th class="p-3">Setor</th>
                            <th class="p-3">Tipo de Liberação / Alçada</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        ${(setores || []).map(s => `
                            <tr>
                                <td class="p-3 font-bold text-slate-900">${escapeHTML(s.nome)}</td>
                                <td class="p-3"><span class="bg-emerald-50 text-emerald-700 font-semibold px-2.5 py-1 rounded-md">${escapeHTML(s.tipo_liberacao)}</span></td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
...
// ==========================================
// 11. GERENCIAMENTO DE USUÁRIOS (PERFIS)
// ==========================================
async function carregarUsuarios(container) {
    ...
    const { data: perfis } = await supabaseClient.from('perfis').select('*').order('nome', { ascending: true });

    container.innerHTML = `
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 w-full">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="font-bold text-slate-900 text-base">Gerenciamento de Usuários</h3>
                    <p class="text-xs text-slate-500">Lista de perfis cadastrados no sistema.</p>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs text-slate-700">
                    <thead class="bg-slate-50 uppercase text-[10px] text-slate-500">
                        <tr>
                            ...
                            <th class="p-3">Identificador / Usuário</th>
                            <th class="p-3">Perfil</th>
                            <th class="p-3">Setor</th>
                            <th class="p-3">Liberação</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        ${(perfis || []).length > 0 ? perfis.map(p => `
                            <tr>
                                <td class="p-3 font-bold text-slate-900">${escapeHTML(p.nome)}</td>
                                <td class="p-3 text-slate-500">${escapeHTML(p.usuario)}</td>
                                <td class="p-3"><span class="px-2 py-0.5 rounded font-bold ${p.perfil === 'ADMIN' ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'}">${escapeHTML(p.perfil)}</span></td>
                                <td class="p-3">${escapeHTML(p.setor || '-')}</td>
                                <td class="p-3">${escapeHTML(p.liberacao_setor || '-')}</td>
                            </tr>
                        `).join('') : '<tr><td colspan="5" class="p-4 text-center text-slate-400">Nenhum perfil encontrado.</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
