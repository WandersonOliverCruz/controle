...
// ==========================================
// NAVEGAÇÃO E RESPONSIVIDADE MOBILE
// ==========================================
...
function fecharMenuMobileSeAberto() {
    if (window.innerWidth < 1024 && menuOpenMobile) {
        toggleMenuMobile();
    }
}
...
function navegarPara(pagina) {
    paginaAtual = pagina;
    construirMenu();
    fecharMenuMobileSeAberto(); // Fecha o menu lateral automaticamente em celulares
...
// ==========================================
// CARREGAMENTO DAS TELAS (RESPONSIVAS)
// ==========================================

// 1. PAINEL PRINCIPAL
async function carregarPainel(container) {
    container.innerHTML = `
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 w-full">
            <div class="bg-white p-5 lg:p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-slate-400">Total de Chamados</p>
                    <h4 id="dash-total-chamados" class="text-xl lg:text-2xl font-bold text-slate-900 mt-1">...</h4>
                </div>
            ...
            <div class="bg-white p-5 lg:p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h4 id="dash-total-equipamentos" class="text-xl lg:text-2xl font-bold text-slate-900 mt-1">...</h4>
                </div>
                <div class="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold">
                    <i class="fa-solid fa-desktop"></i>
                </div>
            </div>
            <div class="bg-white p-5 lg:p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between sm:col-span-2 lg:col-span-1">
                <div>
                    <p class="text-xs font-semibold text-slate-400">Usuários Ativos</p>
                    <h4 id="dash-total-usuarios" class="text-xl lg:text-2xl font-bold text-slate-900 mt-1">...</h4>
                </div>
...
async function carregarUsuarios(container) {
    container.innerHTML = `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100 h-fit">
                ...
                <form id="form-novo-usuario" onsubmit="salvarNovoUsuario(event)" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" id="novo-setor" class="w-full text-xs p-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Ex: TI, RH">
                        </div>
                    </div>
            ...
            <!-- LISTA DE USUÁRIOS (ROLA HORIZONTALMENTE EM CELULARES) -->
            <div class="lg:col-span-2 bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100 overflow-hidden">
...
async function carregarLicencas(container) {
    container.innerHTML = `
        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100 w-full overflow-hidden">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h3 class="font-bold text-slate-900 text-sm">Gestão de Licenças de Software</h3>
                    <p class="text-xs text-slate-500">Controle de chaves e licenças ativas</p>
                </div>
            </div>
            <div id="loading-licencas" class="text-xs text-slate-500">Carregando licenças...</div>
    ...
    if (licencas.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="p-4 text-center text-slate-400">Nenhuma licença cadastrada.</td></tr>`;
        return;
    }
...
async function carregarRelatorios(container) {
    container.innerHTML = `
        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100 w-full">
...
async function carregarEquipamentos(container) {
    container.innerHTML = `<div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100"><h3 class="font-bold text-slate-900 text-sm">Equipamentos</h3></div>`;
}

async function carregarChamados(container) {
    container.innerHTML = `<div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100"><h3 class="font-bold text-slate-900 text-sm">Central de Chamados</h3></div>`;
}

async function carregarMeusChamados(container) {
    container.innerHTML = `<div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100"><h3 class="font-bold text-slate-900 text-sm">Meus Chamados</h3></div>`;
}

async function carregarCategorias(container) {
    container.innerHTML = `<div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100"><h3 class="font-bold text-slate-900 text-sm">Categorias e Subcategorias</h3></div>`;
}

async function carregarSetores(container) {
    container.innerHTML = `<div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-100"><h3 class="font-bold text-slate-900 text-sm">Setores e Liberações</h3></div>`;
}
